...
function task1() {
  ...
  try {
    throw new Error('Oops, something weng wrong!');
  } catch (error) {
...
function task3() {
  // Code here
  try {
    throw new Error('Something is wrong with my badminton');
...
function task4() {
  ...
  try {
    ...
    try {
      throw new Error('Badminton Broken');

    } catch (innererror) {
      console.error('Inner error occured', innererror.message);

    }

  } catch (outererror) {
    console.error('An error occured', outererror.message);
...
task6({ firstName: 'Mehedi' }, 'firstName');
